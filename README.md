# فایتر | داشبورد هوشمند تحلیل پرداخت زرین‌پال

پروژه تیم **فایتر** برای تبدیل داده خام تراکنش‌ها به یک داشبورد مدیریتی قابل‌فهم، قابل‌ردیابی و اقدام‌محور است. سامانه وضعیت پرداخت هر پذیرنده را تحلیل می‌کند، افت‌ها و الگوهای مهم را به شکل بینش نمایش می‌دهد و با یک مشاور هوشمند، پیشنهادهای مبتنی بر داده ارائه می‌کند.

## اعضای تیم

| نام | مسئولیت |
| --- | --- |
| روزبه حسین عسگری | توسعه Backend |
| محمدرضا ندایی | توسعه Frontend |

## مسئله و راه‌حل

حجم بالای تراکنش‌ها باعث می‌شود تشخیص سریع علت افت نرخ موفقیت، شکست پرداخت یا رفتار غیرعادی پذیرنده دشوار باشد. این پروژه داده‌های تراکنش را ingest و تجمیع می‌کند و نتیجه را در قالب شاخص‌های کلیدی، روندها، قیف پرداخت، تحلیل تلاش مجدد و بینش‌های قابل‌توضیح در اختیار کاربر می‌گذارد.

قابلیت‌های اصلی:

- داشبورد فارسی و واکنش‌گرا با نمای کلی شاخص‌ها و روند پرداخت
- تحلیل سلامت پرداخت، قیف تبدیل و تلاش‌های مجدد
- تولید خودکار بینش از تغییرات معنادار داده‌ها
- صفحه Trace برای نمایش شواهد و مسیر محاسبه هر بینش
- مشاور هوشمند با پاسخ داده‌محور و fallback قطعی در صورت قطع سرویس LLM
- احراز هویت JWT و محدودسازی داده‌ها بر اساس دسترسی پذیرنده
- API مستند و قابل‌آزمایش با Swagger
- اجرای کامل Backend، Frontend و PostgreSQL با Docker Compose

## مسیر پیشنهادی دمو برای داوران

پس از انجام راه‌اندازی سریع:

1. داشبورد را در <http://localhost:5173> باز کنید.
2. از نمای کلی، پذیرنده و بازه زمانی را تغییر دهید و روند شاخص‌ها را بررسی کنید.
3. وارد بخش «بینش‌ها» شوید و جزئیات یکی از بینش‌ها را باز کنید.
4. در صفحه Trace، شواهد و منطق محاسبه آن بینش را مشاهده کنید.
5. در بخش «مشاور هوشمند»، گزارش و پیشنهادهای مبتنی بر داده را آزمایش کنید.
6. قرارداد API را در <http://localhost:8000/api/v1/docs> ببینید.

## معماری و فناوری‌ها

- **Backend:** Django، Django Ninja، PostgreSQL و DuckDB
- **Frontend:** React، TypeScript و Vite؛ سرو production با nginx
- **Analytics:** پردازش و نگهداری داده تحلیلی با DuckDB
- **Infrastructure:** Docker و Docker Compose

کل استک با یک دستور Docker اجرا می‌شود و برای اجرای نسخه داوری، نصب Node.js روی سیستم لازم نیست.
